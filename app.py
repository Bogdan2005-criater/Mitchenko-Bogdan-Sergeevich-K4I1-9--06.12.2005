...
from bd_insert import insert_bd
from configs import DATABASE
from calculator_of_product import calculate_product_output
...
import os
app = Flask(__name__)
app.secret_key = "ghjg454jiji45fo98003"
...
def add_material():
    ...
    material_types = conn.execute("SELECT id, type_name FROM MaterialType").fetchall()
    conn.close()
    return render_template("form_material.html", material_types=material_types)
...
def material_suppliers(material_id):
    ...
    conn.close()
    return render_template("suppliers_material.html", 
...
def calculate_product():
    ...
    if request.method == "POST":
        ...
        if errors:
            for error in errors:
                flash(error, "error")
            return render_template("calculator_of_product.html", 
